...
public class Ex2_p532 {
	public static void main(String[] args) {
		...
		System.out.println("현재 월 : " +  (cal.get( Calendar.MONTH)+1)); // 1월이 0부터 시작
		...
		// 주 : 일:1 ~ 토:7
		int week = cal.get( cal.get( Calendar.DAY_OF_WEEK));
		...
		if ( week == Calendar.MONDAY) { System.out.println("토"); }
		...
		System.out.println("종료일 과 시작일 같은 : " + 시작일.isEqual(종료일));
		...
		System.out.println( df.format(1234556.7) );
		...
		dateFormat = new SimpleDateFormat("올해의 D번째 날");
		System.out.println( dateFormat.format(date) );
		
		dateFormat = new SimpleDateFormat("이달의 D번째 날");
		System.out.println( dateFormat.format(date) );
