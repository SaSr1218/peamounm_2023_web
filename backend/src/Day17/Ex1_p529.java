...
public class Ex1_p529 {
	public static void main(String[] args) {
		...
		// 1. Math 클래스 : 수학 계산에 사용할 수 있는 메소드 제공(new 없이 사용)
			// 1. ceil( ) 	: 올림
			// 2. floor( ) 	: 버림
			// 3. round ( ) : 반올림 [ 소수 첫째자리 ] -- String.format(%.소수자리f , 값) : 문자 표현이 된다.
			// 4. max ( 값1 , 값2 ) : 최대값
			// 5. min ( 값1 , 값2 ) : 최소값
		...
		// p.531
			...
			// 배열, 리스트 공통점 : 동일한 자료형의 데이터 여러개 저장
		int[] selectNumber = new int[6];	// int형 변수를 6개 저장할 수 있는 배열
				// vs
		ArrayList< Integer > selectNumber2 = new ArrayList<>(6);
				// 포장 클래스 : 기본자료형-->클래스   [  * 특정 API 클래스 중 기본자료형 사용불가 ]
