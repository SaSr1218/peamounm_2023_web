...
public class Ex2_입력 {
	public static void main(String[] args) {
		...
		// 1. [ p.70 ] 입력
			...
			// 입력 객체 : Scanner scaaner = new Scanner ( System.in );
				//{ }별 1번 선언
		// 1.입력객체
		Scanner scanner = new Scanner(System.in);
		// 2. 입력메소드 -> 변수 저장 [ 자료형 맞춰야함 ! ] 
			// 1. 문자열[ 띄어쓰기 불가능 ] 입력받기 함수
			String 문자열1 	= scanner.next();				System.out.println("문자열1 : " + 문자열1);
			// 2. 문자열[ 띄어쓰기 가능 ] 입력받기 함수 [ .nextLine() 앞에 다른 next() 하나로 취급 ]
			...
			// 4. 한글자 입력받기 함수	.next() 문자열중에서 .charAt(0) 첫글자만 반환
			...
			// 문자열[String 객체] 비교
			String 아이디 = "qwe"; // [ String 객체 연산자 불가능 -> 문자열A.equals("문자열B") ]
