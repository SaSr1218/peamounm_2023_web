...
// 4. 수정 함수
let upindex = -1;	// 수정할 인덱스( -1 = 기본값 인덱스 없다는 뜻 ) // 여러개 { } 에서 동일한 변수로 사용하기 위함! // * 전역변수 *
function onUpdate ( i ) { // f s
	upindex = i // 내가 선택한 i번째 인덱스
	document.querySelector('.updatebox').style.display = 'block' // 1. 수정페이지 보여주기
	
	document.querySelector('.upname').value = studentArray[upindex].name // 2. 선택한 i번째 객체의 속성 데이터를 대입
...
// 1. 번호 2. 이름 3. 국어 4. 영어 5. 수학 6.총점 7.평균 8.순위(총점 기준)


/*		// for 문
	for ( let i = 0 ; i<studentArray.length ; i++ ){ // for s
		html += `<tr>
					<td> ${i+1} </td>
