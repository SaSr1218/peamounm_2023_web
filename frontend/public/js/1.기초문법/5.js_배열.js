/* 
	...
	배열 : 데이터 여러개를 저장
		...
		5. 요소[자료] 추가 : 마지만 변수 뒤에 추가
			배열명.push ( 새로운 요소 )
			배열명.splice ( 추가 할 인덱스, 0, '새로운요소' ) : 해당 인덱스에 새로운 요소 추가 [ 기존에 있던 요소들은 한칸씩 뒤로 이동]
				[추가된 인덱스 뒤로 한칸씩 이동]
			
		6. 배열의 특정 요소 제거
			- 인덱스 이용한 제거
				배열명.splice( 제거할 인덱스의 시작번호, 제거개수, [새로운 값] )
...
배열명.splice( 0, 1 ); console.log( 배열명 )
	// * 한줄에 두개 이상의 명령어 작성시에는 ;(세미콜론)으로 구분하기
// 7. 데이터로 인덱스 찾기
let sindex = 배열명.indexOf('신동엽'); console.log( '신동엽 인덱스 : ' + sindex)
// 8. 데이터로 요소 삭제
배열명.slice( 배열명.indexOf('신동엽'), 1 ); console.log( 배열명 )
// 9. 배열내 요소 데이터 변경
배열명.splice( 0, 1, '서장훈'); 
