...
let burgerList = [ 
	...
	{ name : '치킨킹팩1' , price : 13000 , img : '치킨킹팩.png' , category : '스페셜' } ,
	{ name : '비프불고기버거' , price : 7000, img : '비프불고기버거.png' , category : '와퍼'}
]
let cartList = [  ] // 카트 목록 
let orderList = [  ] // 주문 목록
...
function register(){ // f s
	// 버거등록btn 함수 작동확인
	alert('등록합니다.')
	console.log(등록된버거)
	
	let 버거이름 = document.querySelector('.버거이름').value
	let 카테고리명 = document.querySelector('.카테고리명').value
	let 가격 = document.querySelector('.가격').value
	let 이미지 = document.querySelector('.이미지').value

	let Burger = {name : 버거이름 , category : 카테고리명 , price : 가격, 이미지명 : 이미지}
	등록된버거.push( Burger )
	console.log( 등록된버거 )
		// 1. html 구성
	let html = `<tr>
	...
	for( let i = 0 ; i < 등록된버거.length; i++ ){ // for s
		// 유효성검사 카테고리배열에 존재하는 카테고리만 입력
		if(categoryList.indexOf(Burger.category) != -1){
		html += `<tr>
					<td>${ 등록된버거[i].name }</td>
					<td>${ 등록된버거[i].category }</td>
					<td>${ 등록된버거[i].price }</td>
					<td><img class="b_img" src="${burgerList[i]}"></td>
				</tr>`
				document.querySelector('.버거등록테이블').innerHTML = html
				}else{
					alert('카테고리명이 일치하지 않습니다.');
				}
	} // for e
...
function bprint(){
	...
	for( let i = 0; i < 등록된버거.length; i++ ){
		html2 += `<tr>
					<td>${ i+1 }</td>
					<td>이미지</td>
					<td>${등록된버거[i].name}</td>
					<td>${등록된버거[i].category}</td>
					<td class="price">${등록된버거[i].price}</td>
					<td>
						<button onclick="onDelete( ${i} )">삭제</button>
						<button onclick="pupdate( ${i} )">가격수정</button>
					</td>
				</tr>`
	}
		document.querySelector('.현황등록테이블').innerHTML = html2
}
...
function pupdate(){
	...
	document.querySelector('.price').value = 등록된버거[i].price
	prompt(Number())
}
