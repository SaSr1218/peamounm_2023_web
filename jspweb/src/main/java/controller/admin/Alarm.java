...
public class Alarm {
	...
	@OnMessage // 클라이언트 소켓이 메시지를 보냈을때 매핑[연결]
...
/*
 	...
 	@ServerEndpoint("/alarm/{mig}")	
	public class Alarm {
	
	@OnOpen // 클라이언트 소켓이 들어왔을때 매핑[연결]
	public void 서버입장( Session session ) throws Exception { System.out.println(session); }
	
	@OnClose // 클라이언트 소켓이 나갔을때 매핑[연결]
	public void 서버나가기( Session session ) throws Exception { System.out.println(session); }
	
	@OnError // 클라이언트 소켓이 에러가 발생했을때 매핑[연결]
	public void 서버오류( Session session , Throwable e )  throws Exception { System.out.println(session); }
	
	@OnMessage // 클라이언트 소켓이 메시지를 보냈을때 매핑[연결]
	public void 서버메시지( Session session , String msg ) throws Exception { System.out.println(session); }
